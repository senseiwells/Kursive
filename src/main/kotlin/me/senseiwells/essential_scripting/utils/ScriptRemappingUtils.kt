...
object ScriptRemappingUtils {
    ...
    private fun mapScriptingApi() {
        ...
        this.unmappedModJars.add(scriptingApiJar)
        for (named in MappingType.named()) {
            this.createRemappedModJar(CommonScriptingApi.MOD_ID, scriptingApiJar, named)
        }

        val includes = this.getScriptingApiIncludes(scriptingApi)
        for ((id, include) in includes) {
            this.unmappedModJars.add(include)
    ...
    private fun writeIntermediary2Mojang2YarnMappings() {
        ...
        val intermediary = this.getOrDownloadMappings(this.getOfficial2IntermediaryMappingsPath(), this::downloadIntermediaryMappings)
        val yarn = this.getOrDownloadMappings(this.getIntermediary2YarnMappingsPath(), this::downloadYarnMappings)
        val mojang = this.getOrDownloadMappings(this.getMojang2OfficialMappingsPath(), this::downloadMojangMappings)
    ...
    private fun downloadMojangMappings() {
        ...
        try {
            NetworkingUtils.fetchAsReader(url) { reader ->
                val path = this.getMojang2OfficialMappingsPath()
    ...
    private fun downloadYarnMappings() {
        ...
        try {
            NetworkingUtils.fetchAsStream(url) { stream ->
                val path = this.getIntermediary2YarnMappingsPath()
    ...
    private fun downloadIntermediaryMappings() {
        ...
        try {
            NetworkingUtils.fetchAsStream(url) { input ->
                val path = this.getOfficial2IntermediaryMappingsPath()
                path.outputStream().use { output ->
                    input.transferTo(output)
                }
            }
